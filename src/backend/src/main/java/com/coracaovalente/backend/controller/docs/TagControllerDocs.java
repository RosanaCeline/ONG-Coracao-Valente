...
public interface TagControllerDocs {
    ...
    @Operation(summary = "Lista tags", description = "Método para listar todas as tags")
    @ApiResponse(responseCode = "200", description = "Lista de tags retornada com sucesso")
    @ApiResponse(responseCode = "401", description = "Requisição não autorizada", content = @Content)
    @ApiResponse(responseCode = "403", description = "Acesso negado", content = @Content)
    @ApiResponse(responseCode = "500", description = "Erro inesperado no servidor", content = @Content)
    public ResponseEntity<List<com.coracaovalente.backend.model.animal.Tag>> getTags ();
}
